...
function OrdersPage() {
    ...
    const generatePdf = (ordersToExport)=>{
        ...
        ordersToExport.forEach((order, index)=>{
            ...
            doc.text('INVOICE', pageWidth - 20, 30, {
                align: 'right'
            });
            doc.setFontSize(10);
            ...
            doc.setFont('helvetica', 'normal');
            doc.text(order.id, pageWidth - 20, 45, {
                align: 'right'
            });
            doc.text(formatDateInIST(order.timestamp), pageWidth - 20, 52, {
                align: 'right'
            });
            // Line Separator
            ...
            doc.text(`₹${order.subTotal.toFixed(2)}`, textX, finalY, {
                align: 'right'
            });
            doc.text(`Tax (${(0.1 * 100).toFixed(0)}%):`, subtotalX, finalY + 7, {
                align: 'right'
            });
            doc.text(`₹${order.taxAmount.toFixed(2)}`, textX, finalY + 7, {
                align: 'right'
            });
            ...
            doc.setFontSize(12);
            doc.setFont('helvetica', 'bold');
            doc.text('TOTAL:', subtotalX, finalY + 18, {
                align: 'right'
            });
            doc.text(`₹${order.totalAmount.toFixed(2)}`, textX, finalY + 18, {
                align: 'right'
            });
            // Footer
            doc.setFontSize(10);
...
//# sourceMappingURL=%5Broot%20of%20the%20server%5D__f80c3dac._.js.map